    ...
    with tf.name_scope(name or 'minimize'):
        ...
        def _body(state, value):
            ...
            state.converged.assign(
                ...
                state.tolerance)
            value.history.scatter_add(tf.IndexedSlices(state.objective_value, state.num_iterations))
            return [state, value]
        ...
        logs = collections.namedtuple("logs", ["history"])
        init_logs = logs(**{"history" : tf.Variable(tf.zeros(max_iterations + 1, dtype=tf.float32))})
        ...
        return tf.while_loop(cond=_cond,
                             body=_body,
                             loop_vars=[initial_state, init_logs],
